...
7. **Registration vs refresh asymmetry.** Registration rejects atomically on any unknown provider reference (400 `invalid_request_error`, code `unknown_provider`). Background refresh warns (`event: "refresh_rejected"`, `reason: "unknown_provider"`) and keeps the cached manifest — matching the existing stale-on-error policy so removing a provider degrades loudly, not silently.
