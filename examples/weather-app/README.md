...
A minimal Node/Hono app that registers itself with the Iriguchi gateway, exposes one agent (`weather-bot`) with one `api_call` tool (`get_forecast`) and one inline skill (`weather-jargon`), and serves a static chat UI at `/`.
